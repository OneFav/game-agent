...
TRAIN_PY = dedent(
    ...
    def main() -> int:
        ...
        (output_dir / "training_curves.csv").write_text(
            "step,episode,reward_mean,actor_loss,critic_loss,evaluation_primary\\n"
            "0,0,0.0,0.0,0.0,\\n",
            encoding="utf-8",
        )
...
# ── swarm_combat 策略模板（导入式，引用 reference_policies）─────────────────

SWARM_RULE_POLICY_PY = """\
\"\"\"policies/{policy_id}/policy.py -- 基于 SafeRulePolicyAdapter 的策略

由 policy_designer 自动生成。
导入 src/game_agent/policy_designer/reference_policies/ 参考实现。
\"\"\"
from __future__ import annotations
...
from contracts.policy_protocol import Policy
from game_agent.policy_designer.reference_policies import SafeRulePolicyAdapter


class PolicyClass(Policy):
    \"\"\"适配 SafeRulePolicy 到 Policy ABC 接口的策略类.

    SafeRulePolicy 拥有完整环境状态访问、两阶段穿门策略、多航道分配、
    前视风险检测和刹车机制。
    \"\"\"

    def __init__(self, config: dict[str, Any] | None = None, env_spec: dict[str, Any] | None = None):
        self._adapter = SafeRulePolicyAdapter(config or {{}}, env_spec or {{}})

    def reset(self, seed: int) -> None:
        self._adapter.reset(seed)

    def act(self, obs, agent_id: str, info: Any = None) -> np.ndarray:
        return self._adapter.act(obs, agent_id, info)

    def load(self, checkpoint_path: str) -> None:
        self._adapter.load(checkpoint_path)
    ...
    def get_diagnostics(self) -> dict[str, Any]:
        return self._adapter.get_diagnostics()
"""

SWARM_RULE_TRAIN_PY = """\
\"\"\"policies/{policy_id}/train.py -- 训练入口 (no-op for rule policy).

SafeRulePolicy 为规则策略，不需要训练。
本脚本作为占位符，满足 AutoResearch 的接口要求。
\"\"\"
from __future__ import annotations
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output_dir", required=True, help="Where to write checkpoint")
    parser.add_argument("--max_steps", type=int, default=600, help="Max training steps")
    parser.add_argument("--wall_time_limit", type=int, default=3600, help="Wall time limit (s)")
    parser.add_argument("--resume_from", default=None, help="Resume checkpoint")
    ...
    # 规则策略不需要训练：写入空 checkpoint 占位
    checkpoint = output_dir / "checkpoint.json"
    checkpoint.write_text(json.dumps({{"policy_type": "SafeRulePolicy", "seed": args.seed}}))
    return 0
...
SWARM_RULE_ALGORITHM_CARD = """\
# {policy_id} -- Algorithm Card

## Family
Rule-based safe navigation policy (SafeRulePolicy).

## Compatible Scenarios
3D swarm combat environments (swarm_combat). N vs N multi-agent scenarios
with racer + defender roles, gate layouts, and collision constraints.

## Assumptions
- Global observability (each drone sees all others + all gates)
- Deterministic dynamics (DoubleIntegrator3D or DampedDoubleIntegrator3D)
- Gate pass is the sole scoring mechanism
- RACER drones aim to pass gates; DEFENDER drones escort or intercept

## Input/Output
- Input: full env state (all drone positions/velocities + all gate states)
- Output: 3D acceleration commands [ax, ay, az] per drone
- Actions are clipped to [-max_accel, max_accel] per config

## Training Method
N/A -- rule-based policy. No training required.

## Safety Mechanism
1. Soft separation: drones push away from each other when too close
2. Boundary pushback: drones steer away from field boundaries
3. Forward risk check: simulate N steps ahead; full brake if collision predicted
4. Brake release: resume navigation when speed drops below threshold

## Known Limitations
- Does not learn from experience
- Lane assignment is naive (split by team membership)
- Risk prediction assumes constant-velocity for other drones
- No communication delay or packet loss modeling

## Expected Failure Modes
- High-speed head-on collisions (risk prediction too short)
- Gate congestion with many racers competing for same gate
- Defender-interceptor oscillation between multiple threats

## Computational Requirements
- ~0.1 ms per drone per step (CPU)
- Memory: O(num_drones * lookahead_steps) for risk simulation
"""
