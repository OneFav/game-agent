...
def _compile_scenario(project_root: Path, task: str, task_id: str) -> Path:
    ...
    _run_hook(REPO_ROOT / "hooks" / "post_scenario_compile.py", "--scenario", scenario_dir)
    return scenario_dir
...
def _build_policy(project_root: Path, scenario_dir: Path, policy_id: str) -> Path:
    ...
    _run_hook(REPO_ROOT / "hooks" / "post_policy_submit.py", "--policy", policy_dir)
    return policy_dir
...
def _run_experiment(project_root: Path, scenario_dir: Path, policy_dir: Path, exp_id: str) -> Path:
    ...
    _run_hook(REPO_ROOT / "hooks" / "post_experiment_run.py", "--exp", exp_dir)
    return exp_dir
