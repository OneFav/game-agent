# Algorithm Card: max_space_s44_rule_v1
...
- Representative distinction: `hybrid_action_space`
...
For `S44`, `lifecycle_role_rule` should improve `valid_hybrid_mission_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `hybrid_action_space`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `valid_hybrid_mission_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
