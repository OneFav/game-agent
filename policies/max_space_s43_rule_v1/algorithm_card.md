# Algorithm Card: max_space_s43_rule_v1
...
- Representative distinction: `runtime_role_transition`
...
For `S43`, `lifecycle_role_rule` should improve `role_reassignment_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `runtime_role_transition`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `role_reassignment_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
