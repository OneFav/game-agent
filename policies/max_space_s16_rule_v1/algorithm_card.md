# Algorithm Card: max_space_s16_rule_v1
...
- Representative distinction: `relational_team_objective`
...
For `S16`, `decentralized_team_rule` should improve `formation_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `relational_team_objective`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `formation_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
