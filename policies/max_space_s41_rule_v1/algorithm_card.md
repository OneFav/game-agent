# Algorithm Card: max_space_s41_rule_v1
...
- Scenario: `S41` — 智能体动态加入覆盖
- Task family: `hybrid_mission`
- Representative distinction: `dynamic_agent_spawn`
...
For `S41`, `lifecycle_role_rule` should improve `dynamic_team_coverage_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `dynamic_agent_spawn`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `dynamic_team_coverage_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
