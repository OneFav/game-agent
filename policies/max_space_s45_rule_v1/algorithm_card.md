# Algorithm Card: max_space_s45_rule_v1
...
- Representative distinction: `event_driven_task_graph`
...
For `S45`, `lifecycle_role_rule` should improve `event_chain_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `event_driven_task_graph`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `event_chain_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
