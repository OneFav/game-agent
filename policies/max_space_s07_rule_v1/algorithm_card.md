# Algorithm Card: max_space_s07_rule_v1
...
- Scenario: `S07` — 三维双积分器制动着陆
- Task family: `dynamics`
- Representative distinction: `terminal_state_constraints`
...
For `S07`, `dynamics_aware_pd_rule` should improve `safe_landing_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `terminal_state_constraints`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `safe_landing_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
