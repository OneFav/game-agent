# Algorithm Card: max_space_s01_rule_v1
...
- Scenario: `S01` — 二维静态航路点导航
- Task family: `navigation`
- Representative distinction: `single_agent_continuous_navigation`
...
For `S01`, `bounded_goal_vector_rule` should improve `route_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `single_agent_continuous_navigation`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `route_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
