# Algorithm Card: max_space_s26_rule_v1
...
- Scenario: `S26` — 有限视场搜索与跟踪
- Task family: `sensor_game`
- Representative distinction: `directional_field_of_view`
...
For `S26`, `observation_limited_rule` should improve `target_track_fraction` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `directional_field_of_view`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `target_track_fraction`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
