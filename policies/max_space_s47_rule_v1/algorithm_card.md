# Algorithm Card: max_space_s47_rule_v1
...
- Scenario: `S47` — 五十智能体集群
- Task family: `scale_external`
- Representative distinction: `large_scale_runtime`
...
For `S47`, `scalable_modality_rule` should improve `large_swarm_shape_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `large_scale_runtime`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `large_swarm_shape_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
