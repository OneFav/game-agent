# Algorithm Card: max_space_s34_rule_v1

## Family

Rule-based / `stale_communication_rule`. Learning is unnecessary for this stage because the package is an explicit, deterministic reference used to test protocol conformance and same-seed improvement over a zero-action baseline. It is not a claim that learning or MPC is unnecessary for a final high-fidelity solution.

## Compatible Scenarios

- Suite: `max_space_50_v1`
- Scenario: `S34` — 动态通信拓扑覆盖
- Task family: `communication_game`
- Representative distinction: `dynamic_communication_graph`
...
For `S34`, `stale_communication_rule` should improve `coverage_with_connectivity_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `dynamic_communication_graph`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `coverage_with_connectivity_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.

## Immutable Boundaries

- Keep method `stale_communication_rule` and its rule-based paradigm.
