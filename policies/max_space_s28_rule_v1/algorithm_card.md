# Algorithm Card: max_space_s28_rule_v1
...
- Scenario: `S28` — 带噪传感器状态估计
- Task family: `sensor_game`
- Representative distinction: `sensor_measurement_model`
...
For `S28`, `observation_limited_rule` should improve `safe_landing_rate_under_sensor_noise` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `sensor_measurement_model`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `safe_landing_rate_under_sensor_noise`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
