...
For `S10`, `dynamics_aware_pd_rule` should improve `flight_envelope_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `external_dynamics_adapter`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `flight_envelope_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.

## Immutable Boundaries

- Keep method `dynamics_aware_pd_rule` and its rule-based paradigm.
