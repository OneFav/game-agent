# Algorithm Card: max_space_s30_rule_v1
...
- Scenario: `S30` — CTDE 信息分离
- Task family: `sensor_game`
- Representative distinction: `ctde_information_contract`
...
For `S30`, `observation_limited_rule` should improve `decentralized_coverage_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `ctde_information_contract`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `decentralized_coverage_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
