# Algorithm Card: max_space_s42_rule_v1

## Family

Rule-based / `lifecycle_role_rule`. Learning is unnecessary for this stage because the package is an explicit, deterministic reference used to test protocol conformance and same-seed improvement over a zero-action baseline. It is not a claim that learning or MPC is unnecessary for a final high-fidelity solution.

## Compatible Scenarios

- Suite: `max_space_50_v1`
- Scenario: `S42` — 失效退出编队恢复
- Task family: `hybrid_mission`
- Representative distinction: `agent_failure_and_exit`
...
For `S42`, `lifecycle_role_rule` should improve `post_failure_mission_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `agent_failure_and_exit`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `post_failure_mission_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.

## Immutable Boundaries

- Keep method `lifecycle_role_rule` and its rule-based paradigm.
