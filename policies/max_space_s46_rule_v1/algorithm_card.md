# Algorithm Card: max_space_s46_rule_v1
...
- Scenario: `S46` — 十智能体局部集群
- Task family: `scale_external`
- Representative distinction: `medium_scale_swarm`
...
For `S46`, `scalable_modality_rule` should improve `local_coordination_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `medium_scale_swarm`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `local_coordination_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
