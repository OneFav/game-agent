# Algorithm Card: max_space_s35_rule_v1
...
- Representative distinction: `communication_infrastructure_role`
...
For `S35`, `stale_communication_rule` should improve `mission_completion_with_relay_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `communication_infrastructure_role`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `mission_completion_with_relay_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
