# Algorithm Card: max_space_s17_rule_v1
...
- Representative distinction: `dynamic_relation_graph`
...
For `S17`, `decentralized_team_rule` should improve `formation_transition_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `dynamic_relation_graph`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `formation_transition_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
