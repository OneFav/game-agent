# Algorithm Card: max_space_s15_rule_v1
...
- Representative distinction: `stateful_regions`
...
For `S15`, `role_aware_pursuit_rule` should improve `side_specific_win_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `stateful_regions`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `side_specific_win_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
