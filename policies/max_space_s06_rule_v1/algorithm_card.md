# Algorithm Card: max_space_s06_rule_v1
...
- Scenario: `S06` — 二维单积分器能耗导航
- Task family: `dynamics`
- Representative distinction: `single_integrator_dynamics`
...
For `S06`, `dynamics_aware_pd_rule` should improve `energy_adjusted_success` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `single_integrator_dynamics`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `energy_adjusted_success`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
