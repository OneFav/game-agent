# Algorithm Card: max_space_s25_rule_v1
...
- Scenario: `S25` — 多阶段护航—突防
- Task family: `escort_defense`
- Representative distinction: `multi_phase_mission`
...
For `S25`, `role_aware_escort_defense_rule` should improve `full_mission_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `multi_phase_mission`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `full_mission_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
