# Algorithm Card: max_space_s36_rule_v1
...
- Representative distinction: `broad_initial_state_randomization`
...
For `S36`, `robust_capped_pd_rule` should improve `capture_rate_across_spawn_distribution` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `broad_initial_state_randomization`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `capture_rate_across_spawn_distribution`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
