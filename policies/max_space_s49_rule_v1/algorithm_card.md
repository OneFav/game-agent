# Algorithm Card: max_space_s49_rule_v1
...
- Representative distinction: `multimodal_image_observation`
...
For `S49`, `scalable_modality_rule` should improve `vision_navigation_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `multimodal_image_observation`, observation modality `image`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `vision_navigation_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
