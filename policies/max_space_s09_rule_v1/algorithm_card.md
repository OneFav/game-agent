# Algorithm Card: max_space_s09_rule_v1
...
- Scenario: `S09` — 异构快慢双机协同到达
- Task family: `dynamics`
- Representative distinction: `heterogeneous_dynamics`
...
For `S09`, `dynamics_aware_pd_rule` should improve `synchronized_arrival_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `heterogeneous_dynamics`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `synchronized_arrival_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
