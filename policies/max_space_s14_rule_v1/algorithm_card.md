# Algorithm Card: max_space_s14_rule_v1
...
- Scenario: `S14` — 一追多逃目标选择
- Task family: `pursuit_evasion`
- Representative distinction: `one_to_many_target_selection`
...
For `S14`, `role_aware_pursuit_rule` should improve `weighted_capture_score` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `one_to_many_target_selection`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `weighted_capture_score`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
