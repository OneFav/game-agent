# Algorithm Card: max_space_s31_rule_v1
...
- Scenario: `S31` — 固定两步通信延迟
- Task family: `communication_game`
- Representative distinction: `fixed_message_delay`
...
For `S31`, `stale_communication_rule` should improve `delayed_coordination_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `fixed_message_delay`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `delayed_coordination_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
