# Algorithm Card: max_space_s13_rule_v1
...
- Scenario: `S13` — 三追一协同围捕
- Task family: `pursuit_evasion`
- Representative distinction: `many_to_one_coordination`
...
For `S13`, `role_aware_pursuit_rule` should improve `team_capture_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `many_to_one_coordination`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `team_capture_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
