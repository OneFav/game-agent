# Algorithm Card: max_space_s20_rule_v1

## Family

Rule-based / `decentralized_team_rule`. Learning is unnecessary for this stage because the package is an explicit, deterministic reference used to test protocol conformance and same-seed improvement over a zero-action baseline. It is not a claim that learning or MPC is unnecessary for a final high-fidelity solution.

## Compatible Scenarios

- Suite: `max_space_50_v1`
- Scenario: `S20` — 协同三维穿门竞速
- Task family: `team_cooperation`
- Representative distinction: `team_synchronized_progress`
...
For `S20`, `decentralized_team_rule` should improve `team_ordered_gate_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `team_synchronized_progress`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `team_ordered_gate_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.

## Immutable Boundaries

- Keep method `decentralized_team_rule` and its rule-based paradigm.
