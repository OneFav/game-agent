# Algorithm Card: max_space_s38_rule_v1
...
- Scenario: `S38` — 动力学参数随机化
- Task family: `robustness`
- Representative distinction: `hidden_dynamics_randomization`
...
For `S38`, `robust_capped_pd_rule` should improve `dynamics_robust_tracking_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `hidden_dynamics_randomization`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `dynamics_robust_tracking_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
