# Algorithm Card: max_space_s40_rule_v1
...
- Scenario: `S40` — 训练评估分布偏移
- Task family: `robustness`
- Representative distinction: `frozen_distribution_shift_protocol`
...
For `S40`, `robust_capped_pd_rule` should improve `ood_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `frozen_distribution_shift_protocol`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `ood_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
