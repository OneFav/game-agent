# Algorithm Card: max_space_s11_rule_v1
...
- Scenario: `S11` — 完全观测 1v1 追逃
- Task family: `pursuit_evasion`
- Representative distinction: `two_sided_competition`
...
For `S11`, `role_aware_pursuit_rule` should improve `capture_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `two_sided_competition`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `capture_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
