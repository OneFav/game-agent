# Algorithm Card: max_space_s37_rule_v1
...
- Scenario: `S37` — 随机障碍布局泛化
- Task family: `robustness`
- Representative distinction: `procedural_geometry_randomization`
...
For `S37`, `robust_capped_pd_rule` should improve `unseen_map_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `procedural_geometry_randomization`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `unseen_map_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
