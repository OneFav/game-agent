...
def load_policy_class() -> type:
    spec = importlib.util.spec_from_file_location("tested_vertical_wave_policy_side_effects", POLICY_DIR / "policy.py")
...
def test_act_does_not_mutate_observation_or_write_files() -> None:
    ...
    obs[46:49] = np.array([4.0, 0.0, 1.0], dtype=np.float32)
    original = obs.copy()
