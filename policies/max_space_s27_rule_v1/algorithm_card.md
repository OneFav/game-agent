# Algorithm Card: max_space_s27_rule_v1
...
- Representative distinction: `geometric_occlusion`
...
For `S27`, `observation_limited_rule` should improve `line_of_sight_tracking_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `geometric_occlusion`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `line_of_sight_tracking_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
