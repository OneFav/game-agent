# Algorithm Card: max_space_s24_rule_v1
...
- Scenario: `S24` — 非对称攻防角色
- Task family: `escort_defense`
- Representative distinction: `asymmetric_team_contracts`
...
For `S24`, `role_aware_escort_defense_rule` should improve `breakthrough_fraction` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `asymmetric_team_contracts`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `breakthrough_fraction`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
