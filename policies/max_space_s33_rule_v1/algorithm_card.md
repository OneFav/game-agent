# Algorithm Card: max_space_s33_rule_v1
...
- Representative distinction: `bandwidth_constrained_messages`
...
For `S33`, `stale_communication_rule` should improve `detections_per_communication_budget` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `bandwidth_constrained_messages`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `detections_per_communication_budget`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
