# Algorithm Card: max_space_s02_rule_v1
...
- Scenario: `S02` — 二维动态障碍导航
- Task family: `navigation`
- Representative distinction: `dynamic_obstacles`
...
For `S02`, `bounded_goal_vector_rule` should improve `goal_reach_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `dynamic_obstacles`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `goal_reach_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
