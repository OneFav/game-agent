# Algorithm Card: max_space_s23_rule_v1
...
- Scenario: `S23` — 多护航者对多拦截者
- Task family: `escort_defense`
- Representative distinction: `many_to_many_adversarial_coupling`
...
For `S23`, `role_aware_escort_defense_rule` should improve `protected_asset_fraction` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `many_to_many_adversarial_coupling`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `protected_asset_fraction`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
