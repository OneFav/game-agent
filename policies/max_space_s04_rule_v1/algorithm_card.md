# Algorithm Card: max_space_s04_rule_v1
...
- Scenario: `S04` — 三维移动目标交会
- Task family: `navigation`
- Representative distinction: `moving_objective`
...
For `S04`, `bounded_goal_vector_rule` should improve `rendezvous_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `moving_objective`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `rendezvous_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
