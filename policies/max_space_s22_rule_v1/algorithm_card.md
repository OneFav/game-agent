# Algorithm Card: max_space_s22_rule_v1
...
- Scenario: `S22` — 多护航者对单拦截者
- Task family: `escort_defense`
- Representative distinction: `multi_agent_protection`
...
For `S22`, `role_aware_escort_defense_rule` should improve `protected_route_completion_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `multi_agent_protection`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `protected_route_completion_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
