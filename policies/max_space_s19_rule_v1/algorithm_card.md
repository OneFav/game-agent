# Algorithm Card: max_space_s19_rule_v1
...
- Scenario: `S19` — 多目标任务分配
- Task family: `team_cooperation`
- Representative distinction: `hybrid_assignment_and_control`
...
For `S19`, `decentralized_team_rule` should improve `feasible_task_value_ratio` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `hybrid_assignment_and_control`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `feasible_task_value_ratio`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
