# Algorithm Card: max_space_s05_rule_v1
...
- Scenario: `S05` — 拓扑航路网络导航
- Task family: `navigation`
- Representative distinction: `graph_world`
...
For `S05`, `bounded_goal_vector_rule` should improve `valid_route_success_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `graph_world`, observation modality `graph`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `valid_route_success_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
