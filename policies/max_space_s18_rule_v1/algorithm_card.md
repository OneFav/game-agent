# Algorithm Card: max_space_s18_rule_v1
...
- Scenario: `S18` — 分布式区域覆盖
- Task family: `team_cooperation`
- Representative distinction: `spatial_field_objective`
...
For `S18`, `decentralized_team_rule` should improve `coverage_ratio_at_budget` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `spatial_field_objective`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `coverage_ratio_at_budget`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
