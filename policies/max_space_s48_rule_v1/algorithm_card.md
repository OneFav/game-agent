# Algorithm Card: max_space_s48_rule_v1
...
- Scenario: `S48` — 动态图观测集群
- Task family: `scale_external`
- Representative distinction: `graph_observation_space`
...
For `S48`, `scalable_modality_rule` should improve `graph_observation_passage_rate` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `graph_observation_space`, observation modality `graph`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `graph_observation_passage_rate`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
