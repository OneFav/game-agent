# Algorithm Card: max_space_s50_rule_v1

## Family

Rule-based / `scalable_modality_rule`. Learning is unnecessary for this stage because the package is an explicit, deterministic reference used to test protocol conformance and same-seed improvement over a zero-action baseline. It is not a claim that learning or MPC is unnecessary for a final high-fidelity solution.

## Compatible Scenarios

- Suite: `max_space_50_v1`
- Scenario: `S50` — 外部仿真端到端接入
- Task family: `scale_external`
- Representative distinction: `full_external_simulator_adapter`
...
For `S50`, `scalable_modality_rule` should improve `external_adapter_reproducibility` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `full_external_simulator_adapter`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `external_adapter_reproducibility`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.

## Immutable Boundaries

- Keep method `scalable_modality_rule` and its rule-based paradigm.
