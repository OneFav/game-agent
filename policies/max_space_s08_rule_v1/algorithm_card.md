# Algorithm Card: max_space_s08_rule_v1
...
- Scenario: `S08` — 阻尼动力学持续跟踪
- Task family: `dynamics`
- Representative distinction: `damped_dynamics`
...
For `S08`, `dynamics_aware_pd_rule` should improve `trajectory_tracking_score` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `damped_dynamics`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `trajectory_tracking_score`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
