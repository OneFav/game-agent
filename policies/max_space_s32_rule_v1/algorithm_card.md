# Algorithm Card: max_space_s32_rule_v1
...
- Representative distinction: `stochastic_packet_loss`
...
For `S32`, `stale_communication_rule` should improve `formation_success_under_packet_loss` over `max_space_zero_v1` on the same seeds while preserving hard constraints. Suspected bottlenecks are `stochastic_packet_loss`, observation modality `vector`, and the gain/damping safety trade-off.
...
Mode is `initial`; primary utility is the cross-seed mean of scenario evaluator metric `formation_success_under_packet_loss`. Baseline and candidate use the same evaluator metric and seeds. Reward components are never substituted for the score.
